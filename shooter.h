...
public:
    ...
    //gives all the values of the shooter 
	void shooter_information(float &shooter_x,float &shooter_y, float &shooter_radius, float &shooter_width, float &shooter_height, float &shooter_head_y)
	{
		shooter_x=get_body_center_x();
		shooter_y=get_body_center_y();
		shooter_radius=get_head_radius();
		shooter_width=get_body_width();
		shooter_height=get_body_height();
		shooter_head_y=get_head_center_y();
	}

    //changes the shooter color when the bubble touches it
    void changeshootercol(bool to_green){
        Color shootercolor;
        if (to_green){
            shootercolor=COLOR(0,255,0);
        }
        else{
            shootercolor=COLOR(0,255,255);
        }
        body.setColor(shootercolor);
        body.setFill(true);
        head.setColor(shootercolor);
        head.setFill(shootercolor);
    }
};
